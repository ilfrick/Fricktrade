...
- Fixed docker-socket-proxy being stopped during market shutdown, which broke autoheal and prevented healthwatch from restarting services.
...
- Live loop optimization: market_state now includes `last_bar_ts`, `_market_state_from_df` avoids extra pandas work, and an optional `data.process_on_new_bar_only` gate skips symbols without new bars.
